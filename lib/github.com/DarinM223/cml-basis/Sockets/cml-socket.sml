...
struct
  ...
  local
    fun acceptNB' sock =
      case Socket.acceptNB sock of
        SOME (sock', addr) => SOME (PreSock.mkSock sock', addr)
      | NONE => NONE
    fun accept' sock =
      let val (sock', addr) = Socket.accept sock
      in (PreSock.mkSock sock', addr)
      end
  in
    fun acceptEvt (s as PS.CMLSock {sock, ...}) =
      CML.guard (fn () =>
  ...
  fun recvVecEvt (s as PS.CMLSock {sock, ...}, n) =
    CML.guard (fn () =>
      case Socket.recvVecNB (sock, n) of
        (SOME res) => CML.alwaysEvt res
