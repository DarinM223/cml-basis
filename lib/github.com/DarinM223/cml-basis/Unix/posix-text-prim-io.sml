...
struct
  ...
  fun posFns (closed, fd) =
    if (isRegFile fd) then
      let
        ...
        fun verifyPos () =
          let val curPos = PIO.lseek (fd, Position.fromInt 0, PIO.SEEK_CUR)
          in pos := curPos; curPos
          end
      in
        ignore (verifyPos ());
        { pos = pos
  ...
  fun mkReader {fd, name} =
    let
      val iod = PF.fdToIOD fd
      val lockMV = SV.mVarInit ()
      fun withLock f x =
        (SV.mTake lockMV; (Syscall.doSyscall f x) before SV.mPut (lockMV, ()))
      ...
      fun incPos k =
        pos := Position.+ (!pos, Position.fromInt k)
      fun blockWrap f x =
        (if !closed then raise IO.ClosedStream else (); f x)
      val readEvt = IOManager.ioEvt (OS.IO.pollIn
        (Option.valOf (OS.IO.pollDesc iod)))
      fun eventWrap f x =
        CML.withNack (fn nack =>
          ( if !closed then raise IO.ClosedStream else ()
          ; case (SV.mTakePoll lockMV) of
              NONE =>
                let
                  val replV = SV.iVar ()
                in
                  CML.spawn (fn () =>
                    CML.select
                      [CML.wrap (readEvt, fn _ => SV.iPut (replV, ())), nack]);
                  CML.wrap (SV.iGetEvt replV, fn _ => f x)
                end
            | (SOME _) =>
                CML.wrap (readEvt, fn _ => (SV.mPut (lockMV, ()); f x))
          (* end case *)
          ))
      fun readVec n =
        let
      ...
      fun close () =
        if !closed then () else (closed := true; PIO.close fd)
      val isReg = isRegFile fd
      fun avail () =
        if !closed then SOME 0
        else if isReg then SOME (PF.ST.size (PF.fstat fd) - !pos)
        else NONE
    in
      TextPrimIO.RD
        { name = name
  ...
  fun openRd name =
    mkReader {fd = PF.openf (name, PIO.O_RDONLY, PF.O.flags []), name = name}
  ...
  fun mkWriter {fd, name, appendMode, chunkSize} =
    let
      ...
      fun write put arg =
        (ensureOpen (); put (fd, arg))
      val writeEvt = IOManager.ioEvt (OS.IO.pollOut
        (Option.valOf (OS.IO.pollDesc iod)))
      fun eventWrap f x =
        CML.withNack (fn nack =>
          ( if !closed then raise IO.ClosedStream else ()
          ; case (SV.mTakePoll lockMV) of
              NONE =>
                ...
                in
                  CML.spawn (fn () =>
                    CML.select
                      [CML.wrap (writeEvt, fn _ => SV.iPut (replV, ())), nack]);
                  CML.wrap (SV.iGetEvt replV, fn _ => f x)
                end
            | (SOME _) =>
                CML.wrap (writeEvt, fn _ => (SV.mPut (lockMV, ()); f x))
          (* end case *)
          ))
      fun close () =
        if !closed then () else (closed := true; PIO.close fd)
  ...
  val standardMode = PF.S.flags
    [ (* mode 0666 *) PF.S.irusr
    , PF.S.iwusr
    , PF.S.irgrp
    , PF.S.iwgrp
    , PF.S.iroth
  ...
  fun stdIn () = mkReader {fd = PF.stdin, name = "<stdIn>"}
  ...
  fun stdOut () =
    mkWriter
      ...
      , name = "<stdOut>"
      , appendMode = false
      , (* Bug!  Should check! *) chunkSize = bufferSzB
      }
  ...
  fun strReader src =
    let
      val lockMV = SV.mVarInit ()
      fun withLock f x =
        (SV.mTake lockMV; f x before SV.mPut (lockMV, ()))
        handle ex => (SV.mPut (lockMV, ()); raise ex)
      val pos = ref 0
      val closed = ref false
      fun checkClosed () =
      ...
      fun readA asl =
        ...
        in
          ...
          CharArraySlice.copyVec
            {src = CharVectorSlice.slice (src, p, SOME m), dst = buf, di = i};
          m
        end
      fun getPos () =
        (checkClosed (); Position.fromInt (!pos))
      fun setPos p =
        ( checkClosed ()
