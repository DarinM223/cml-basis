...
struct
  ...
  structure UDP =
  struct
    fun socket () = CML_GenericSock.socket (inetAF, Socket.SOCK.dgram)
    fun socket' proto =
  ...
  structure TCP =
  struct
    fun socket () = CML_GenericSock.socket (inetAF, Socket.SOCK.stream)
    fun socket' proto =
      CML_GenericSock.socket' (inetAF, Socket.SOCK.stream, proto)
    (* tcp control options *)
    fun getNODELAY (PreSock.CMLSock {sock, ...}) = INetSock.TCP.getNODELAY sock
