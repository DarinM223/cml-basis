...
struct
  ...
  fun mkReader ch =
    let
      ...
      val isClosedEvt = CML.wrap (SV.iGetEvt closedFlg, fn () =>
        raise IO.ClosedStream)
      datatype req = RD of (int * unit CML.event * V.vector CML.chan) | CLOSE
      val reqCh = Mailbox.mailbox ()
      fun readVecEvt 0 =
            CML.alwaysEvt (V.fromList [])
        | readVecEvt n =
            if (n < 0) then
              ...
              CML.withNack (fn nack =>
                ...
                in
                  ...
                  CML.choose [CML.recvEvt replCh, isClosedEvt]
                end)
      fun readArrEvt asl =
        let
        ...
        in
          CML.wrap (readVecEvt n, fn v =>
            (A.copyVec {dst = buf, di = i, src = v}; V.length v))
        end
      fun close () = Mailbox.send (reqCh, CLOSE)
      fun getData NONE =
            let val v = CML.recv ch
            in if (V.length v > 0) then v else getData NONE
            end
        | getData (SOME v) = v
      fun server buf =
        (case (Mailbox.recv reqCh) of
           RD (n, nack, replCh) =>
             let
               val v = getData buf
             in
               if (V.length v > n) then
                 let
                   val v' = vextract (v, 0, SOME n)
                 in
                   CML.select
                     [ CML.wrap (nack, fn () => server (SOME v))
                     , CML.wrap (CML.sendEvt (replCh, v), fn () =>
  ...
  fun mkWriter ch =
    let
      val closedFlg = SV.iVar ()
      val closedEvt = CML.wrap (SV.iGetEvt closedFlg, fn () =>
        raise IO.ClosedStream)
      val ch' = CML.channel ()
      fun buffer () =
        CML.select
          [ CML.wrap (CML.recvEvt ch', fn v =>
              (if (V.length v > 0) then CML.send (ch, v) else (); buffer ()))
          , closedEvt
          ]
      fun writeVecEvt arg =
        let
        ...
        in
          CML.choose
            [closedEvt, CML.wrap (CML.sendEvt (ch', v), fn () => V.length v)]
        end
      fun writeArrEvt arg =
        let
          val v = AS.vector arg
