...
functor TextIOFn
  (
   ...
   where type PrimIO.writer = TextPrimIO.writer): CML_TEXT_IO =
struct
  ...
  struct
    ...
    and in_buffer =
      IBUF of
        { basePos: pos option
        , more: more SV.mvar
        , (* when this cell is empty, it means that *)
    ...
    and info =
      INFO of
        ...
        , (* points to the more cell of the last buffer *) cleanTag: CleanIO.tag
        }

    fun infoOfIBuf (IBUF {info, ...}) = info
    fun chunkSzOfIBuf buf =
      let val INFO {reader = PIO.RD {chunkSize, ...}, ...} = infoOfIBuf buf
      in chunkSize
      end
     ...
     *)
    fun extendStream (readFn, mlOp, buf as IBUF {more, info, ...}) =
      (let
         val INFO {getPos, tail, ...} = info
         val basePos = getPos ()
         val chunk = readFn (chunkSzOfIBuf buf)
       in
         if (V.length chunk = 0) then
           (SV.mPut (more, NOMORE); EOF)
         else
           let
             val newMore = SV.mVar ()
             val buf' = IBUF
               {basePos = basePos, data = chunk, more = newMore, info = info}
           in
       ...
       handle ex => (SV.mPut (more, NOMORE); inputExn (info, mlOp, ex)))
    ...
    fun generalizedInput getBuf =
      let
        fun get (ISTRM (buf as IBUF {data, ...}, pos)) =
          let
            val len = V.length data
          in
            if (pos < len) then
              (vecExtract (data, pos, NONE), ISTRM (buf, len))
            else
              (case (getBuf buf) of
    ...
    fun terminate (info as INFO {tail, cleanTag, ...}) =
      let
        val m = SV.mGet tail
      in
        case SV.mTake m of
          (m' as MORE _) => (SV.mPut (m, m'); terminate info)
        | TERMINATED => SV.mPut (m, TERMINATED)
        | _ => (CleanIO.removeCleaner cleanTag; SV.mPut (m, TERMINATED))
      (* end case *)
      end
    ...
    fun input1 (ISTRM (buf, pos)) =
      let
        val IBUF {data, more, ...} = buf
      in
        if (pos < V.length data) then
          SOME (vecSub (data, pos), ISTRM (buf, pos + 1))
        else
          let
            fun get (MORE buf) =
                  input1 (ISTRM (buf, 0))
    ...
    fun inputN (ISTRM (buf, pos), n) =
      let
        fun join (item, (list, strm)) = (item :: list, strm)
        fun inputList (buf as IBUF {data, ...}, i, n) =
          let
            val len = V.length data
            val remain = len - i
          in
            if (remain >= n) then
              ([vecExtract (data, i, SOME n)], ISTRM (buf, i + n))
            else
              join (vecExtract (data, i, NONE), nextBuf (buf, n - remain))
          end
        and nextBuf (buf as IBUF {more, data, ...}, n) =
          let
    ...
    fun inputAll (strm as ISTRM (buf, _)) =
      let
        ...
        val bigInput = generalizedInput (getBuffer (bigChunk, "inputAll"))
        fun loop (v, strm) =
    ...
    fun canInput (strm as ISTRM (buf, pos), amount) =
      let
        (******
        		  ...
        		  (* end case *))
        ******)
        fun tryInput (buf as IBUF {data, ...}, i, n) =
          let
            val len = V.length data
            val remain = len - i
          in
            if (remain >= n) then SOME n else nextBuf (buf, n - remain)
          end
        and nextBuf (IBUF {more, ...}, n) =
          let
      ...
      in
        if (amount < 0) then raise Size else tryInput (buf, pos, amount)
      end
    ...
    fun closeIn (ISTRM (buf, _)) =
      closeInInfo (infoOfIBuf buf)
    fun endOfStream (ISTRM (buf as IBUF {more, ...}, pos)) =
      (case SV.mTake more of
         (next as MORE _) => (SV.mPut (more, next); false)
       | next =>
           let
    ...
    fun mkInstream arg =
      let val (tag, strm as ISTRM (IBUF {info, ...}, _)) = mkInstream' arg
      in CleanIO.rebindCleaner (tag, fn () => closeInInfo info); strm
      end
    fun getReader (ISTRM (buf, pos)) =
      let
      ...
      in
        terminate info;
        if (pos < V.length data) then
          (reader, V.concat (vecExtract (data, pos, NONE) :: getData more))
        else
          (reader, V.concat (getData more))
      end
    ...
    *)
    ...
    fun filePosIn (ISTRM (buf, pos)) =
      (case buf of
         IBUF {basePos = NONE, info, ...} =>
           inputExn (info, "filePosIn", IO.RandomAccessNotSupported)
       | IBUF {basePos = SOME base, info, ...} =>
           let
             val INFO {reader = PIO.RD rd, readVec, ...} = info
           in
             case (#getPos rd, #setPos rd) of
               (SOME getPos, SOME setPos) =>
                 let
                   val tmpPos = getPos ()
                   fun readN 0 = ()
                     | readN n =
                         (case V.length (readVec n) of
                            0 =>
                              inputExn
    ...
    *)
    ...
    fun inputLine (ISTRM (buf as IBUF {data, ...}, pos)) =
      let
        fun join (item, (list, strm)) = (item :: list, strm)
        fun nextBuf (isEmpty, buf as IBUF {more, data, ...}) =
          let
            fun last () =
        ...
        and scanData (buf as IBUF {data, ...}, i) =
          let
            val len = V.length data
            fun scan j =
              if (j = len) then
                join (vecExtract (data, i, NONE), nextBuf (false, buf))
              else if (vecSub (data, j) = #"\n") then
                ([vecExtract (data, i, SOME (j + 1 - i))], ISTRM (buf, j + 1))
              else
                scan (j + 1)
          in
            scan i
        ...
        val (data, strm) =
          if (V.length data = pos) then nextBuf (true, buf)
          ...
          fn arg =>
            CML.guard (fn () =>
              ...
              in
                ...
                CML.wrap
                  (SV.iGetEvt resV, fn (RES x) => x | (EXN ex) => raise ex)
              end)
        end
    in
    ...
    fun lockAndChkClosedOut (strmMV, mlOp) =
     ...
     *)
    fun lineBufCopyVec (src, srcI, srcLen, dst, dstI) =
      let
        val stop = srcI + srcLen
        fun cpy (srcI, dstI, lb) =
          if (srcI < stop) then
            let
              val c = vecSub (src, srcI)
            in
    ...
    fun blockBufCopyVec (src, srcI, srcLen, dst, dstI) =
      ( AS.copyVec
          {src = VS.slice (src, srcI, SOME srcLen), dst = dst, di = dstI}
      ; false
      )
            ...
            if (dataLen >= bufLen) then
              ...
              in
                if (avail < dataLen) then
                  let
                    val _ = AS.copyVec
                      {src = VS.slice (v, 0, SOME avail), dst = buf, di = i}
                    val _ = flushAll ()
                  ...
                  in
                    pos := i + dataLen;
                    if (needsFlush orelse (avail = dataLen)) then flush ()
                    else ()
                  end
              end
          end
      ...
      in
        ...
        | IO.LINE_BUF =>
            ...
            in
              ...
              pos := i';
              if ((i' = A.length buf) orelse (isNL elem)) then
                flushBuffer (strmMV, strm, "output1")
              else
                ()
            end
        | IO.BLOCK_BUF =>
    ...
    fun flushOut strmMV =
      let val strm = lockAndChkClosedOut (strmMV, "flushOut")
      in flushBuffer (strmMV, strm, "flushOut"); SV.mPut (strmMV, strm)
      end

    fun closeOut strmMV =
      let
        val
          (strm as OSTRM {writer = PIO.WR {close, ...}, closed, cleanTag, ...}) =
          SV.mTake strmMV
      in
        if !closed then
          ()
      ...
      let
        fun iterate (f, size, subslice) =
          let
            fun lp sl =
              if size sl = 0 then ()
              else let val n = f sl in lp (subslice (sl, n, NONE)) end
          in
            lp
          end
    ...
    fun mkOutstream arg =
      let val (tag, strm) = mkOutstream' arg
      in CleanIO.rebindCleaner (tag, fn () => closeOut strm); strm
      end

    fun getWriter strmMV =
      let
    ...
    datatype out_pos =
      OUTP of {pos: PIO.pos, strm: outstream}

    fun getPosOut strmMV =
      let
    ...
    fun setBufferMode (strmMV, mode) =
      let
        val (strm as OSTRM {bufferMode, ...}) =
          lockAndChkClosedOut (strmMV, "setBufferMode")
      in
        if (mode = IO.NO_BUF) then flushBuffer (strmMV, strm, "setBufferMode")
        else ();
        bufferMode := mode;
        SV.mPut (strmMV, strm)
      end
    fun getBufferMode strmMV =
      let
  ...
  (** Input operations **)
  fun input strm =
    let val (v, strm') = StreamIO.input (SV.mTake strm)
  ...
  fun inputAll (strm: instream) =
    let val (v, strm') = StreamIO.inputAll (SV.mTake strm)
    in SV.mPut (strm, strm'); v
    end
  ...
  local
    datatype 'a result = RES of 'a | EXN of exn
    fun sendEvt (ch, v) =
      CML.sendEvt (ch, RES v)
    fun sendExnEvt (ch, exn) =
      CML.sendEvt (ch, EXN exn)
    fun recvEvt ch =
      CML.wrap (CML.recvEvt ch, fn (RES v) => v | (EXN exn) => raise exn)
    fun doInput inputEvt (strm: instream) nack =
      let
        ...
        fun inputThread () =
          let
            ...
            fun handleInput (result, strm'') =
              CML.select
                [ CML.wrap (sendEvt (replyCh, result), fn _ =>
                    SV.mPut (strm, strm''))
                , nackEvt
                ]
          in
            (CML.select [CML.wrap (inputEvt strm', handleInput), nackEvt])
            handle exn =>
              CML.select
                [ CML.wrap (sendExnEvt (replyCh, exn), fn _ =>
                    SV.mPut (strm, strm'))
                , nackEvt
                ]
          end
  ...
  in
    fun input1Evt (strm: instream) =
      let
        fun inputEvt (strm: StreamIO.instream) =
          CML.wrap
    ...
    fun inputAllEvt Strm =
      CML.withNack (doInput StreamIO.inputAllEvt Strm)
  end (* local *)

  fun canInput (strm, n) =
    StreamIO.canInput (SV.mGet strm, n)
  ...
  fun endOfStream strm =
    StreamIO.endOfStream (SV.mGet strm)
  (*
  ...
  *)
  ...
  (** Output operations **)
  fun output (strm, v) =
    StreamIO.output (SV.mGet strm, v)
  fun output1 (strm, c) =
    StreamIO.output1 (SV.mGet strm, c)
  fun flushOut strm =
    StreamIO.flushOut (SV.mGet strm)
  fun closeOut strm =
    StreamIO.closeOut (SV.mGet strm)
  fun getPosOut strm =
    StreamIO.getPosOut (SV.mGet strm)
  ...
  fun outputSubstr (strm, ss) =
    StreamIO.outputSubstr (SV.mGet strm, ss)
  fun openString src =
    mkInstream (StreamIO.mkInstream (OSPrimIO.strReader src, empty))
    handle ex =>
      raise IO.Io {function = "openIn", name = "<string>", cause = ex}
  ...
  local
    ...
    fun mkStdIn rebind =
      let val (tag, strm) = SIO.mkInstream' (OSPrimIO.stdIn (), empty)
      in if rebind then CleanIO.rebindCleaner (tag, dummyCleaner) else (); strm
      end
    fun mkStdOut rebind =
      let
      ...
      in
        if rebind then CleanIO.rebindCleaner (tag, fn () => SIO.flushOut strm)
        else ();
        strm
      end
    fun mkStdErr rebind =
  ...
  in
    ...
    fun print s =
      let
        val strm' = SV.mTake stdOut
      in
        StreamIO.output (strm', s);
        StreamIO.flushOut strm';
        SV.mPut (stdOut, strm')
      end

    fun scanStream scanFn =
      let
        ...
        fun doit strm =
          ...
          in
            case scan instrm of
              NONE => NONE
            | SOME (item, instrm') => (setInstream (strm, instrm'); SOME item)
          (* end case *)
          end
      in
        doit
