...
struct
  ...
  local structure SV = SyncVar val lockV = SV.mVarInit ()
  in
    fun lock () = SV.mTake lockV
    fun unlock () = SV.mPut (lockV, ())
    fun protect f x =
      if RunCML.isRunning () then
        let
          val _ = lock ()
  ...
  fun clean when =
    let
      ...
      fun doCleaner (_, _, f) =
        CML.select
          [CML.joinEvt (CML.spawnc f when), CML.timeOutEvt (Time.fromSeconds 1)]
    (*DEBUG
  ...
  datatype item = ITEM of {key: string, init: unit -> unit, shut: unit -> unit}

  val chanList = ref ([] : item list)
  val mboxList = ref ([] : item list)
  val serverList = ref ([] : item list)

  fun unlogItem l name =
    let
      fun f [] = raise Unlog
        | f ((x as ITEM {key, ...}) :: r) =
            if (name = key) then r else (x :: (f r))
    in
      l := f (!l)
    end

  fun appInit l =
    List.app (fn ITEM {init, ...} => init ()) (List.rev (!l))

  fun unlogAll () =
    (chanList := []; mboxList := []; serverList := [])

  val unlogChannel = protect (unlogItem chanList)
  fun logChannel (name, ch) =
    let
      fun f () = ignore ch
    (* TODO: add resetChan to MLton? *)
  ...
  fun logMailbox (name, mb) =
    let
      fun f () = ignore mb
    (* TODO: add resetMbox to MLton? *)
  ...
  fun shutdownServers () =
    let
      fun shut (ITEM {shut, ...}) =
        CML.select
          [CML.joinEvt (CML.spawn shut), CML.timeOutEvt (Time.fromSeconds 2)]
    in
      app shut (!serverList)
    end
  ...
  fun cleanChannels _ =
    (appInit chanList; appInit mboxList)
