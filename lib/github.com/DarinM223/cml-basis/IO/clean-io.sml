...
struct
  ...
  fun rebindCleaner (t, close) =
    let
      fun f [] =
            raise Fail "rebindCleaner: tag not found"
        | f (x :: r) =
            let val t' = getTag x
            in if (t' = t) then {tag = t, close = close} :: r else x :: f r
            end
    in
  ...
  fun removeCleaner t =
    let
      fun f [] = [] (* should we raise an exception here? *)
        | f (x :: r) =
            if (getTag x = t) then r else x :: f r
    in
      SV.mPut (cleaners, f (SV.mTake cleaners))
    end
